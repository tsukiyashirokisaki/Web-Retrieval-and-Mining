...
for i in range(400):
    ...
    for (j,k) in random.sample(pos_train,num_ps):
        ...
        for a in range(args.r):
            while (j,l) in pos_train :
                l= random.randint(0,num_item-1)
    ...
    if Map>best_Map:
        best_Map=Map
        torch.save([user,item],"model/%d/bpr/%.3f_%d_%d_%d.pkl"%(num_lat,Map,int(-np.log10(args.lr)),int(-np.log10(args.w)),args.r))
